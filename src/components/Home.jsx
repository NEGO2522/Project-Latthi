            ...
            <div className="mt-8 pt-6 border-t border-gray-100">
              <div className="flex items-center">
                <div className="flex -space-x-2 sm:-space-x-3">
                  {[1, 2, 3, 4].map((item) => (
                    <div key={item} className="relative w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12">
                      <img 
                        src={`https://randomuser.me/api/portraits/${item % 2 === 0 ? 'men' : 'women'}/${item * 10}.jpg`}
                        alt={`Customer ${item}`}
                        className="w-full h-full rounded-full border-2 border-white object-cover shadow-sm"
                        onError={(e) => {
                          e.target.onerror = null;
                          e.target.src = `https://ui-avatars.com/api/?name=Customer+${item}&background=random`;
                        }}
                      />
                      {item === 4 && (
                        <div className="absolute -bottom-1 -right-1 bg-indigo-600 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center font-bold">+</div>
                      )}
                    </div>
                  ))}
                </div>
                <div className="ml-4">
                  <p className="text-sm text-gray-600">Join <span className="font-medium text-gray-900">10,000+</span> satisfied customers</p>
